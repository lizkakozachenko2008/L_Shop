...
export const Icon = ({ name, size = 24, className = '' }: IconProps): SVGElement => {
  ...
  // ✅ ИСПРАВЛЕНИЕ: Разбиваем строку классов и фильтруем пустые значения
  // .filter(Boolean) удалит пустые строки, которые возникают при ''.split(' ')
  const classes = className.split(' ').filter(Boolean);
  if (classes.length > 0) {
    svg.classList.add(...classes);
  }

  // Словарь иконок (SVG paths)
